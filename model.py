...
def buy_sell_regions(df, model): #Function uses the raw data to identify the general by and sell regions 
    ...
    ohe = OneHotEncoder(categories = [['Buy', 'Hold', 'Sell']], sparse = False)#Define the targeted categories before conversion to a sutable form for the ML model
    ...
    return decoded.reshape(-1), df #Reshape the prediction array to a 1D matrix and return it together with the data used before the prediction
...
def distinct_buy_sell(data, model): #Identifies distinct buy sell point in the data based on the buy sell regions predicted above
    ...
    #Analysing predictions from the identified regions to develop suitable feature for the distinct model and saving them to the dataframe
    ...
    y = ohe.fit_transform(data[['Distinct_Action']])#Converting the targets to a form sutable for the ML to interpate diferent categories
    decoded = ohe.inverse_transform(results)#Convert the prediction array back to the respective categories
